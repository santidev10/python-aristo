...
for file in file_list:
     if "iso" in file:
          ...
          subprocess.run(["sudo", "cp", "-f", iso_path, "Aristocrat-VLT/platform/packages"])
          subprocess.run(["sudo", "mount", "-o", "loop", iso_path, "/mnt/efs/disk"])
          # print(iso_path)
     if "manifest" in file:
          manif_file = file.strip()
          manif_path = retail_dir + manif_file
          subprocess.run(["sudo", "cp", "-f", manif_path, "Aristocrat-VLT/platform/packages"])   
